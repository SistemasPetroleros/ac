...
if ($oSoap->fault) {
	...
	fwrite($file,"\n ID SOLICITUD:".$idSolicitud."\n ENTRADA ANULA DISPENSA:".date('Y-m-d H:i:s')."-".$param."\n\n");
    fclose($file);	
    ...
    if ($oSoap->fault) {
        ...
        if ($err) {        // Muestra el error
            ...
            if ($result['resultado'] == "false") {
				...
				//LOG ERRORES
				$file=fopen("logAnulaciones.txt","a");
				$entradaFile='ANULACION DE DISPENSA CON ERRORES:'.date('Y-m-d H:i:s')." - ".$error."\n";
				...
				//LOG	
				$file=fopen("logAnulaciones.txt","a");
				$entradaFile="ANULACION DISPENSA CON EXITO. ENTRA A ACTULIZAR ITEMS Y INSERTAR ESTADOS\n";
				fwrite($file,$entradaFile);
				fclose($file);
