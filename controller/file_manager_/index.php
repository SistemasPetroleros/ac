...
//$file = 'adjuntos/'.$_REQUEST['file'] ?: '.';

$idSolicitud = isset($_GET['idSolicitud']) ? $_GET['idSolicitud'] : -1;
$tipo = isset($_GET['tipo']) ? $_GET['tipo'] : '';

if ($idSolicitud > 0) {
	setcookie("tempFqwerty", $idSolicitud, time() + 3600 * 24);
} else {
	$idSolicitud = $_COOKIE["tempFqwerty"];
}
if (!$idSolicitud > 0) {
	exit;
}
...
function agregarEstado($archivo, $tipo, $detalle = '')
{
	global $idSolicitud;
	...
	if ($tipo == "M") {
		...
		$observacion = 'Archivo adjunto: ' . $archivo;
		if (strlen($detalle) > 0) {
...
if (isset($_GET['adjuntosMateriales'])){
	chdir('adjuntosMateriales');
	
	$adjuntos='adjuntosMateriales';
...
if (!is_dir($idSolicitud)) {
	//chdir($file);
	@mkdir($idSolicitud);
}

chdir($idSolicitud);
//$file = 'adjuntos/'.$idSolicitud.'/'.$_REQUEST['file'] ?: '.';
	...
	if (is_dir($file)) {
		...
		foreach ($files as $entry) if (!is_entry_ignored($entry, $allow_show_folders, $hidden_patterns)) {
			...
			$result[] = [
				...
				'size' => $stat['size'],
				'name' => basename($i),
				'path' => $adjuntos.'/' . $idSolicitud . '/' . preg_replace('@^\./@', '', $i),
				'is_dir' => is_dir($i),
		...
		usort($result, function ($f1, $f2) {
			$f1_key = ($f1['is_dir'] ?: 2) . $f1['mtime'];
			$f2_key = ($f2['is_dir'] ?: 2) . $f2['mtime'];
...
// Determina la ruta base según la solicitud
if (isset($_GET['adjuntosMateriales'])) {
	$adjuntos = 'adjuntosMateriales';
	// Suponiendo que 'adjuntosMateriales' se encuentra en un directorio de tu servidor
	$basePath = "C:/archivosAC/{$adjuntos}";
} else {
	$adjuntos = 'adjuntos';
	// Suponiendo que 'adjuntos' también está en tu servidor
	$basePath = "C:/archivosAC/{$adjuntos}";
}

// Verifica si el directorio del idSolicitud existe, si no lo crea
$idSolicitudPath = $basePath . '/' . $idSolicitud;
if (!is_dir($idSolicitudPath)) {
	@mkdir($idSolicitudPath, 0777, true);  // Crea el directorio si no existe
}

// Cambiar al directorio correcto
chdir($idSolicitudPath);

// Si 'do' es 'list', lista los archivos
if ($_GET['do'] == 'list') {
		...
		foreach ($files as $entry) {
			if (!is_entry_ignored($entry, $allow_show_folders, $hidden_patterns)) {
				...
				$result[] = [
					...
					'size' => $stat['size'],
					'name' => basename($i),
					'path' => 'files/'.$adjuntos . '/' . $idSolicitud . '/' . preg_replace('@^\./@', '', $i),  // Genera la ruta accesible desde la URL
					'is_dir' => is_dir($i),
					'is_deleteable' => $allow_delete && ((!is_dir($i) && is_writable($directory)) ||
						(is_dir($i) && is_writable($directory) && is_recursively_deleteable($i))),
					'is_readable' => is_readable($i),
	...
	$prefijo = ((isset($_GET['externo']) or isset($_POST['externo'])) ? 'Externo-' : 'Privado-');
	$res = move_uploaded_file($_FILES['file_data']['tmp_name'], $file . '/' . $prefijo . $_FILES['file_data']['name']);


	agregarEstado($_FILES['file_data']['name'], $tipo);
	...
	//echo "xxxxxxx";
	$finfo = finfo_open(FILEINFO_MIME_TYPE); //ACA FALLA
	...
	if (strpos($filename, 'Privado-') === 0) {
		$new_filename = substr($filename, strlen('Privado-'));
		agregarEstado($new_filename, $tipo, 'Configurado como Publico');
		rename($filename, $new_filename);
	} else {
		rename($filename, 'Privado-' . $filename);
		agregarEstado('Privado-' . $filename, $tipo, 'Configurado como Privado');
	}
...
function is_entry_ignored($entry, $allow_show_folders, $hidden_patterns)
{
	...
	$hidden_patterns_privado = ['Privado-*'];
	if (isset($_GET['externo'])) {
		foreach ($hidden_patterns_privado as $pattern) {
			if (fnmatch($pattern, $entry)) {
...
<head>
	...
	<style>
		...
		.privado {
			background: url(data:image/png;base64,) no-repeat scroll 0px 5px;
	...
	<script>
		...
		$(function() {
			...
			$('#table').on('click', '.privado', function(data) {
				$.post("?<?= (($_GET['adjuntosMateriales'] > 0) ? 'adjuntosMateriales=1&' : '') ?><?= (($_GET['externo'] > 0) ? 'externo=1&' : '') ?><?= (($_GET['tipo'] == 'M') ? 'tipo=M&' : '') ?>", {
					'do': 'privado',
					file: $(this).attr('data-file'),
					xsrf: XSRF
				}, function(response) {
			...
			<?php if ($allow_upload) : ?>
				...
				function uploadFile(file) {
					...
					xhr.open('POST', '?<?= (($_GET['adjuntosMateriales'] > 0) ? 'adjuntosMateriales=1&' : '') ?><?= (($_GET['externo'] > 0) ? 'externo=1&' : '') ?><?= (($_GET['tipo'] == 'M') ? 'tipo=M&' : '') ?>');
					xhr.onload = function() {
						$row.remove();
						list();
					};
			...
			function list() {
				...
				$.get('?do=list&<?= (($_GET['adjuntosMateriales'] > 0) ? 'adjuntosMateriales=1&' : '') ?><?= (($_GET['externo'] > 0) ? 'externo=1&' : '') ?>file=' + hashval, function(data) {
					$tbody.empty();
					...
					if (data.success) {
						...
						!data.results.length && $tbody.append('<tr><td class="empty" colspan=5>No existen archivos adjuntos.</td></tr>')
			...
			function renderFileRow(data) {
				...
				var $link = $('<a target="_blank" class="name" />')
				...
				var $dl_link = $('<a/>').attr('href', '?do=download&<?= (($_GET['adjuntosMateriales'] > 0) ? 'adjuntosMateriales=1&' : '') ?>file=' + encodeURIComponent(data.name))
					.addClass('download').text('Descargar');

				var $dl_ocultar = '';

				<?php if (!isset($_GET['externo'])) { ?>

					var textPrivado = 'Cambiar a Privado';
					if (data.name.startsWith('Privado-')) {
						textPrivado = 'Cambiar a Publico';
					}
					$dl_ocultar = $('<a href="#" />').attr('data-file', data.path).addClass('privado').text(textPrivado);
					if (data.name.startsWith('Externo-')) {
						$dl_ocultar = '<p style="float: right;">Agregado por Prestador</p>';
					}
				...
				var $html = $('<tr />')
					...
					.append($('<td/>').append($dl_link).append($dl_ocultar).append(data.is_deleteable ? $delete_link : ''))
				return $html;
			}
	...
	<div id="top">
		...
		<?php if ($allow_upload) : ?>
			...
			<div id="file_drop_target">
				Arrastrar y soltar archivos a subir
				<b>o</b>
