...
function agregarEstado($archivo, $tipo)
{
	global $idSolicitud;
		...
		$(function() {
			...
			<?php if ($allow_upload) : ?>
				...
				function uploadFile(file) {
					...
					xhr.open('POST', '?<?=(($_GET['adjuntosMateriales'] > 0)?'adjuntosMateriales=1':'')?>');
					xhr.onload = function() {
						$row.remove();
						list();
					};
			...
			function list() {
				...
				$.get('?do=list&<?=(($_GET['adjuntosMateriales'] > 0)?'adjuntosMateriales=1&':'')?>file=' + hashval, function(data) {
					$tbody.empty();
