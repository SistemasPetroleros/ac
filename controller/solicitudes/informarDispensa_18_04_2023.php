			...
			$result2=json_encode($result);
			   ...
			   if($result2 == false){
				   ...
				   $file=fopen("logDispensa.txt","a");
					...
					$entradaFile=">>>>>> Request:".date('Y-m-d H:i:s')." - ".htmlspecialchars($oSoap->request, ENT_QUOTES)."\n\n--------------------\n\n";
					fwrite($file,$entradaFile);
					$entradaFile=">>>>>> Response:".date('Y-m-d H:i:s')." - ".htmlspecialchars($oSoap->response, ENT_QUOTES)."\n\n--------------------\n\n";
					fwrite($file,$entradaFile);
                    fclose($file);
			   
                    
                    $file=fopen("logDispensaDebug.txt","a");
					$entradaFile=">>>>>>>>>>Debug:".date('Y-m-d H:i:s')."\n\n - ".htmlspecialchars($oSoap->getDebug(), ENT_QUOTES)."\n\n--------------------\n\n";
					fwrite($file,$entradaFile);
                    fclose($file);
            ...
            if ($oSoap->fault) {
                ...
                if ($err) {        // Muestra el error
                  ...
                  //  print_r($result);
				   //LOG	
					$file=fopen("logDispensa.txt","a");
					$entradaFile="RESULTADO 2:".date('Y-m-d H:i:s')." - ".$result['resultado']."\n"."ID ASOCIADO:".$result['codigoTransaccion']."\n\n--------------------\n\n";
					fwrite($file,$entradaFile);
					fclose($file);

              

                    //if ($result['resultado']=="true" or $result['resultado']==true) {
					  $exito=strval($result['resultado']); ///CONSULTAR DANI POR ESTO
                      if ($exito == "true") {
                        //si se realizo con éxito la dispensa
						
						//LOG	
						$file=fopen("logDispensa.txt","a");
						$entradaFile="ENTRA A ACTULIZAR ITEMS Y INSERTAR ESTADOS\n";
						fwrite($file,$entradaFile);
						fclose($file);
                        ...
                        if ($idObj) {
                            ...
                            if (!array_key_exists('0', $result['errores'])) {
                                $listError = "<b>Error " . $result['errores']['_c_error'] . ":</b> " . $result['errores']['_d_error'];
                            } else {
                                $errores = $result['errores'];
                                for ($i = 0; $i < sizeof($errores); $i++) {

                                    $listError .= "<b>Error " . $errores[$i]['_c_error'] . ":</b> " . $errores[$i]['_d_error'] . "<br><br>";
                                }
                            }
