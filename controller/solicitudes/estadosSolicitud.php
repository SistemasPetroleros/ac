...
if (
	...
	(
		...
		($tipo == 29 and  $estadoActual == '30') or
		($tipo == 7 and  $estadoActual == '8') or
		($tipo == 7 and  $estadoActual == '13')
	)
) {
	...
	$seguir = true;
	//De NUEVO A PENDIENTE AUDITORIA, VERIFICO OBLIGATORIOS
	if ($tipo == 2 and  $estadoActual == '1') {
		...
		while ($x = mysqli_fetch_array($proveedores)) {
			array_push($arrayProv, $x['idProveedor']);
			$ok = 1;
		}
	...
	//si lo obligatorio esta OK, continuar
	if ($seguir == true) {
		...
		if ($estadoActual == 4) {
			//Enviar Correo a Proveedores (Deshabilitado por testing 03-01-23)
			//se habilita 21-09-23 requerimiento leo
...
$mostrarBtn = true;
if ($estadoActual == 7) {
	$itemsTraza = new SolicitudesItemTraza();
	//existen items de traza recepcionados?
	$existe = $itemsTraza->existenItems($idSolicitud);
	if ($existe == 1) {
		#si existen, no muestro boton dispensar sin trazabilidad 	
		$mostrarBtn = false;
	}
}
