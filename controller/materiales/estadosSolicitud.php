...
if (
	...
	(
		...
		($tipo == 36 and ($estadoActual == '31' or $estadoActual == '32' or $estadoActual == '34' or $estadoActual == '37')) or
		($tipo == 37 and  $estadoActual == '35') or
		($tipo == 32 and  $estadoActual == '38') or
		($tipo == 38 and  $estadoActual == '32') or
		($tipo == 43 and  $estadoActual == '49') or
		($tipo == 44 and  $estadoActual == '43') or
		($tipo == 48 and  $estadoActual == '37') or
		($tipo == 43 and  $estadoActual == '37') or
		($tipo == 49 and  $estadoActual == '48')
	...
	if ($tipo == 32 and  $estadoActual == '31') {
		//si estado actual NUEVO, y pasa a PENDIENTE DE AUDITORIA
		...
		while ($x = mysqli_fetch_array($proveedores)) {
			array_push($arrayProv, $x['id_proveedores']);
			$ok = 1;
		}
	...
	if ($seguir == true) {
		...
		if ($tipo == 32) {
			...
			for($i=0;$i<count($arrayProv);$i++){
				if (!$csi->existenItemsCotizaciones($idSolicitud, $arrayProv[$i])) {
					$csi->insertarItemsCotizacionesPorQuery($idSolicitud, $arrayProv[$i]);
				}
		...
		if ($tipo == 33 or $tipo == 36 or $tipo==48 or $tipo==49) {

			$csp = new materiales_cotizacion_solic_prov();
			$r = $csp->SelectAllCotizaciones($idSolicitud);

			while ($rowc = mysqli_fetch_assoc($r)) {

				$idCotizacion = $rowc['id'];

				if ($tipo == 33) {
					$descEstado = 'RECHAZADO AUDITORIA';
					$idEstadoCot = 40;
				}
				if ($tipo == 36) {
					$descEstado = 'ANULADO';
				...
				if ($tipo == 48) {
					$descEstado = 'PENDIENTE DE ENTREGA';
					$idEstadoCot = 50;
				}
				...
				//crear estado ANULADO/Rechazado
				$estadoCot = new materiales_cotizaciones_estados();
				$estadoCot->setid_estados($idEstadoCot);
				$estadoCot->setid_cotizacion($idCotizacion);
				$estadoCot->setobservaciones('Cambio de estado Cotizacion #' . $idCotizacion . ' de la solicitud #' . $idSolicitud . " a " . $descEstado . ".<br/> <b>MOTIVO:</b>" . $observacion);
				$estadoCot->Create();


				//Anulo/rechazo la cotizacion
		...
		if($tipo == 37){
			//Enviar Correo Orden de Compra a Proveedores Aprobados
			include_once('enviarEmailsOCProveedores.php');
