...
WHERE AF.BAGFMNROAF='" . $_POST['dniBeneficiario'] . "' AND AF.OGORCODIGO=1                  
      AND (To_Char(AF.BAGFMFECBA,'yyyy-mm-dd')='0001-01-01' OR To_Char(AF.BAGFMFECBA,'yyyy-mm-dd') IS NULL OR  To_Char(AF.BAGFMFECBA,'yyyy-mm-dd')>= To_Char(CURRENT_DATE,'yyyy-mm-dd'))";  /*AND BAT.BATIEFEINI=(SELECT MAX(BAT1.BATIEFEINI)
                                                                  FROM BATIEMP BAT1
...
if( $rowSia=@oci_fetch_assoc($query)) //si existe afiliado en SIA
{
   ...
   $apellidosSia=($rowSia['APELLIDO']);
   $nombresSia=($rowSia['NOMBRES']);
...
$p['idUsuario']=$_SESSION['idUsuario'];
$arrayPuntosDispensa = $puntosdispensa->SelectAll($p);
