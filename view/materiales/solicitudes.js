...
function traerHistorial() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/materiales/traerHistorial.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        data: { 'dni': dni, 'idSolicitudEx': idSolicitud },
        success: function (datos) {
            $('#historial').html(datos);

            $('#thistorialSolicitud').DataTable({
                "responsive": true,
                "paging": true,
                "order": [
                    [0, "desc"]
...
function abrirModalEstadosSolicitud(idEstado, idSolicitud) {
    ...
    if (idEstado == "32") {
        ...
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert alert-warning" role="alert"> ' +
        ...
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert alert-warning" role="alert"> ' +
            '¿Confirma cambiar el estado de la Solicitud a PENDIENTE DE ENTREGA?' +
            '</div>';
    }
    else if (idEstado == "49") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert alert-warning" role="alert"> ' +
            '¿Confirma cambiar el estado de la Solicitud a ENTREGADO?' +
            '</div>';
    }
...
function onChangeStatusSolicitud() {
    ...
    if ((nuevoEstado == 'obs' && observacion.length > 0) || nuevoEstado == '31' || nuevoEstado == '32' || nuevoEstado == '33' || nuevoEstado == '34' || nuevoEstado == '35' || nuevoEstado == '36' || nuevoEstado == '37' || nuevoEstado == '38' || nuevoEstado == '43' || nuevoEstado == '44'|| nuevoEstado == '48' || nuevoEstado == '49') {
        NProgress.start();
        var duracion = 20000;
        type = 'POST'
        $.ajax({
...
function buscarSolicitudes() {
    ...
    $.ajax({
        ...
        async: true,
        timeout: duracion,
        data: datastring+'&buscaEstado='+$('#buscaEstado').val(),
        success: function (datos) {
...
function generarReporte() {
    var parametros = $("#formulario").serialize();
    parametros= parametros+'&buscaEstado='+$('#buscaEstado').val();
    var url = 'controller/materiales/reporteFacturacion.php';
    window.open(url + '?' + parametros, "Reporte Cotizaciones");
}
...
function verHistorialEmails() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/materiales/cargarEmailsEnviados.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/materiales/reenviarEmailsProveedores.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function guardarCabecera(index){


    bootbox.confirm({
        message: "¿Confirma gurdar la información?",
        buttons: {
            confirm: {
                label: 'Sí',
                className: 'btn-primary'
        ...
        callback: function (result) {
            if (result) {
                ...
                var idSolicitud = $("#idSolicitud").val();
                var idProveedor= $("#idProveedorCab"+index).val();
                var validezPropuesta= $("#validezPropuesta"+index).val();
                var plazo_entrega_dias= $("#plazo_entrega_dias"+index).val();
                var incluye_flete= $("#incluye_flete"+index).val();
                var condiciones_pago= $("#condiciones_pago"+index).val();
                var observaciones= $("#observaciones"+index).val();
                var fecha_visualizacion= $("#fecha_visualizacion"+index).val();
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/materiales/guardarCabeceraCotizacion.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
                    data: { 'idSolicitud': idSolicitud, 'idProveedor':idProveedor,'validezPropuesta':validezPropuesta, 'plazo_entrega_dias': plazo_entrega_dias, 'incluye_flete': incluye_flete, 'condiciones_pago':condiciones_pago, 'observaciones': observaciones, 'fecha_visualizacion': fecha_visualizacion  },
                    success: function (data) {


                        if (data === "1")
                            notificar('Se ha guardado la información correctamente!');
                        else
                            notificar('Se ha producido un error, por favor intentente nuevamente!');
