function buscarCotizaciones() {
    ...
    console.log(datastring + '&buscaEstado=' + $('#buscaEstado').val());
...
function traerEstadosCotizacion(idTipoSolicitud) {
    ...
    $.ajax({
        ...
        async: true,
        timeout: duracion,
        data: { 'idSolicitud': idSolicitud, 'idCotizacion': idCotizacion, 'idTipoSolicitud': idTipoSolicitud },
        success: function (datos) {
...
function validaGuarda(tipo) {
    ...
    if (frm.checkValidity()) {
        ...
        elementos.forEach((elemento) => {
            if (elemento.value === "") {
                $('#' + elemento.id).attr('style', 'border:1px solid red;');

            }
            else {
                $('#' + elemento.id).removeAttr('style');
            }
        });
...
function guardarCotizacion(tipo) {
    ...
    $.ajax({
        ...
        async: true,
        timeout: duracion,
        data: datastring + '&idSolicitud=' + idSolicitud + '&idCotizacion=' + idCotizacion + '&idProveedor=' + idProveedor + '&tipo=' + tipo + '&idTipoSolicitud=' + idTipoSolicitud,
        success: function (data) {
            ...
            if (!isNaN(data)) {
                if (tipo == 'B') {
                    ...
                    traerItems();
                    traerEstadosCotizacion(idTipoSolicitud);
...
function traerAdjuntos(control, idSolicitud, idTipoSolicitud = 0) {

    if (idTipoSolicitud == 1)
        var datos = '<iframe src="controller/file_manager/index.php?idSolicitud=' + idSolicitud + '&adjuntos=1&tipo=M&externo=1" style="position: relative;top: 0;left: 0;bottom: 0;right: 0;width: 100%;height: 100%; min-height:500px;frameborder="0" scrolling="yes"">';
    else {
        if (idTipoSolicitud > 1)
...
function onChangeStatusSolicitud() {
    ...
    if ((nuevoEstado == 'obs' && observacion.length > 0) || nuevoEstado == '31' || nuevoEstado == '32' || nuevoEstado == '33' || nuevoEstado == '34' || nuevoEstado == '35' || nuevoEstado == '36' || nuevoEstado == '37' || nuevoEstado == '38' || nuevoEstado == '48' || nuevoEstado == '49') {
        NProgress.start();
        var duracion = 20000;
        type = 'POST'
        $.ajax({
            ...
            async: true,
            timeout: duracion,
            data: { 'idSolicitud': idSolicitud, 'observacion': observacion, 'tipo': nuevoEstado, 'cnt': miliseg(), 'idCotizacion': idCotizacion },
            success: function (datos) {
                //   alert(datos);
                NProgress.set(0.9);
                $('#estado').html(datos);
...
function generarReporte() {
    ...
    bootbox.confirm({
        message: '<h4><b>Resumen para Facturar:</b> Por favor, seleccione una <b>FECHA DESDE/HASTA</b> y estados <b>APROBADA Y PARCIALMENTE APROBADA</b>. </h4>',
        buttons: {
            confirm: {
                label: 'Continuar',
                className: 'btn-success'
        ...
        callback: function (result) {
            if (result) {
                ...
                var parametros = $("#formulariocot").serialize();
                parametros = parametros + '&buscaEstado=' + $('#buscaEstado').val();
                var url = 'controller/cotizaciones/reporteCierre.php';
