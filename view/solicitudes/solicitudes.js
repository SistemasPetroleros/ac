function iniciarSolicitud() {
    ...
    if (dniBeneficiario.length > 5 && dniBeneficiario > 180000 && dniBeneficiario < 99999999) {
...
function guardarSolicitud() {
    ...
    if (puntoDispensa > 0 && idPersona > 0) {
        ...
        $.ajax({
            ...
            async: true,
            timeout: duracion,
            data: { 'idSolicitud': idSolicitud, 'idPersona': idPersona, 'puntoDispensa': puntoDispensa, 'telefono': telefono, 'email': email, 'observaciones': observaciones, 'esB24': esB24, 'esSur': esSur, 'esOncologico': esOncologico, 'fecha_vigencia_cotiz': fecha_vigencia_cotiz, 'cnt': miliseg() },
            success: function (datos) {
...
function abrirModalEstadosSolicitud(idEstado, idSolicitud) {
    ...
    if (idEstado == "2") {
        ...
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿Confirma que el pedido ha sido Recibido?' +
            '</div>';
    } else if (idEstado == "7") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
        ...
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿Enviado a ADM de Farmacias?' +
            '</div>';
    } else if (idEstado == "14") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿En proceso de revisión por ADM. de farmacias?' +
            '</div>';
    } else if (idEstado == "15") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿Auditado por ADM. de farmacias?' +
            '</div>';
    } else if (idEstado == "23") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿Solicitud Pagada?' +
        ...
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert   btn-dark" role="alert"> ' +
            '¿Desea devolver los Productos de la solicitud actual a la Droguería?' +
            '</div>' +
            '<div class="alert alert-warning" role="alert"> ' +
            'Ingrese una Observación de ser necesaria. La solciitud pasará a estado DEVUELTO A DROGUERIA.' +
            '</div>';
    }
    else if (idEstado == "29") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert btn-dark" role="alert"> ' +
            '¿Confirma pasar a la Solicitud a estado CONTROL DE FACTURACION ALTO COSTO APROBADO?' +
            '</div>';
    } else if (idEstado == "30") {
        document.getElementById('mensajeEstados').innerHTML = '<div class="alert btn-dark" role="alert"> ' +
            '¿Confirma pasar a la Solicitud a estado CONTROL DE FACTURACION ALTO COSTO PENDIENTE?' +
            '</div>';
    }
...
function onChangeStatusSolicitud() {
    ...
    if ((nuevoEstado == 'obs' && observacion.length > 0) || nuevoEstado == '2' || nuevoEstado == '3' || nuevoEstado == '4' || nuevoEstado == '6' || nuevoEstado == '7' || nuevoEstado == '8' || nuevoEstado == '9' || nuevoEstado == '1' || nuevoEstado == '13' || nuevoEstado == '14' || nuevoEstado == '15' || nuevoEstado == '23' || nuevoEstado == '26' || nuevoEstado == '27' || nuevoEstado == '28' || nuevoEstado == '29' || nuevoEstado == '30') { //([2, 3, 4, 6, 7, 8, 9].includes(nuevoEstado))
        NProgress.start();
        var duracion = 20000;
        type = 'POST'
        $.ajax({
...
function traerAdjuntos() {
    var idSolicitud = $("#idSolicitud").val();
    var datos = '<iframe src="controller/file_manager/index.php?idSolicitud=' + idSolicitud + '" style="position: relative;top: 0;left: 0;bottom: 0;right: 0;width: 100%;height: 100%; min-height:500px;frameborder="0" scrolling="yes"">';
    $('#adjuntos').html(datos);
...
function abrirModalSolicitudRecep(idEstado) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/recepcionProdSolicitud.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function traerTrazaProductos() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/trazabilidadSolicitud.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
/*function buscar_e_informar(idSolicitud) {
    ...
    if (confirm('Los productos marcados como trazables se informarán al AMNAT. Desea continuar?')) {
        ...
        if (validado) {
            ...
            type = 'POST'
            $.ajax({
                url: 'controller/solicitudes/buscarRecepcionarProductos.php',
                type: type,
                dataType: 'text',
                async: true,
                timeout: duracion,
...
function abrirEstados(idSolicitud, idItem, idItemTraza) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/abrirModalEstadosTraza.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function habilitarDispensa(idSolicitud) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/abrirModalhabilitarDispensa.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function revertirEstado(idSolicitud) {

    var duracion = 180000;
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/abrirModalRevertirEstado.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        ...
        success: function (datos) {
            ...
            $('#divModalRevEst').html(datos);
            $('#modalRevEstado').modal({ backdrop: 'static', keyboard: true, show: true });
...
function verificarRecepcion(idEstado) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/verificarRecepcion.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        ...
        success: function (datos) {
            if (datos.trim() == "1")
                ...
                notificar('Recepecione o anule los productos en estado PENDIENTE.');
...
/*function cambiarEstado(idEstado){
    ...
    type = 'POST'
    $.ajax({
            url: 'controller/solicitudes/cambiarEstado.php',
            type: type,
            dataType: 'text',
            async: true,
            timeout: duracion,
...
function cambiarEstado(idEstado) {
    ...
    // alert('idEstado: '+idEstado+ ' - idSolicitud='+idSolicitud )
...
/*function traerDispensaProductos() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/dispensaSolicitud.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
/*function informarDispensas(idSolicitud) {
    ...
    if (confirm('Los productos marcados como trazables se informarán al AMNAT como DISPENSADOS. Desea continuar?')) {
        ...
        type = 'POST'
        $.ajax({
            url: 'controller/solicitudes/dispensarProductos.php',
            type: type,
            dataType: 'text',
            async: true,
            timeout: duracion,
...
function cerrarDispensa(idSolicitud) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/abrirModalCerrarDispensa.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function verificarDispensa(idEstado) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/verificarDispensa.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function imprimirDispensa(idSolitud) {
    ...
    var form = "controller/solicitudes/reporteDispensa.php";
...
function buscarNoConfirmados() {
    ...
    if ((serie.trim()).length == 0 && (remito.trim()).length == 0) {
        ...
        type = 'POST'
        $.ajax({
            url: 'controller/solicitudes/buscarNoRecepcionados.php',
            type: type,
            dataType: 'text',
            async: true,
            timeout: duracion,
...
function buscarInformados() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/buscarRecepcionados.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function abrirModalQR() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/seleccionarConQR.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function buscarProductoQR() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/decifrarQR.php?data=1',
        type: type,
        data: { 'qr': qr },
        success: function (response) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/verificarGtines.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function informarRecepcion() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/informarNoConfirmados.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        ...
        success: function (data) {
            ...
            if (data.trim() == "") {
                bootbox.alert('Se informaron los productos. Verifique estado y registros de errores en la lista de abajo.');
                buscarInformados();
                buscarNoConfirmados();
            }
            else
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/anularTransaccionRecepcion.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function informarDispensas(idSolicitud) {
    ...
    bootbox.confirm({
        message: "<h4>¿Confirma la Dispensación de los productos seleccionados?</h4><br/><div class=\"alert alert-warning\" role=\"alert\">Si al finalizar la ejecución los productos no apaercen en la lista de Productos Dispensados, por favor, haga clic en la columna Estado de la lista Productos a Dispensar para ver el registro de errores.</div>",
        buttons: {
            confirm: {
                label: 'Sí',
                className: 'btn-primary'
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/informarDispensa.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    //     timeout: duracion,
...
function buscarProductosDispensa(idSolicitud) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/buscarProductosDispensa.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/anularTransaccionDispensa.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function habilitarRecepcion(idSolicitud) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/abrirModalhabilitarRecepcion.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function verificarVolverRecep(idEstado) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/verificarVolverRecep.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function descargarExcel() {
    ...
    form = "controller/solicitudes/generarExcel.php";
...
function traerRemitos() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/solicitudRemitos.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function agregarRemitoModal(id, idSolicitud, op) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/agregarRemitoSolicitudModal.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
function guardarRemito(idSolicitud, op) {
    ...
    if (nroRemito == "")
        ...
        type = 'POST'
        $.ajax({
            url: 'controller/solicitudes/guardarRemitoSolicitud.php',
            type: type,
            dataType: 'text',
            async: true,
            timeout: duracion,
...
function guardarDocsRemito(idSolicitud, idRemito) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/guardarDocsRemitos.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/eliminarSolicitudRemito.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function verificarCheckLists(idSolicitud) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/verificarCheckList.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
...
/*function hideDiv() {
    var idSolicitud = $("#idSolicitud").val();
    if (document.getElementById('esPublico').checked) {

        var duracion = 180000;
        type = 'POST'
        $.ajax({
            url: 'controller/solicitudes/verificarProveedorSolicitud.php',
            type: type,
            dataType: 'text',
            async: true,
            timeout: duracion,
            ...
            success: function (ok) {
                if (ok.trim() == 1) {
                    $('#asigProv').attr('style', 'display:none;');
                    modificarEsPublico(idSolicitud);

                }
                else {
                    notificar('No puede ser pública, existen proveedores asociados.');
                    document.getElementById('esPublico').checked = false;
                }
        ...
        modificarEsPublico(idSolicitud);
        $('#asigProv').removeAttr('style');
    }
}

function modificarEsPublico(idSolicitud) {

    var esPublico = document.getElementById('esPublico').checked;

    var duracion = 180000;
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/modificarPrivadaSolicitud.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        data: { 'idSolicitud': idSolicitud, 'esPublico': esPublico },
        success: function (resp) {
            if (resp.trim() == 0) notificar('Disculpe, ha ocurrido un problema. Intente nuevamente.');
...
function addAllProveedores(accion) {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/asignarTodosProveedoresSolicitud.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        data: { 'idSolicitud': idSolicitud, 'accion': accion },
        success: function (datos) {
            document.getElementById('proveedores').innerHTML = datos;
...
function buscarCotizaciones() {
    ...
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/buscarProveedoresCotizaciones.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
        ...
        callback: function (result) {
            if (result) {
                if ($('#cantAprobada').val() > 0) {
                    ...
                    type = 'POST'
                    $.ajax({
                        url: 'controller/solicitudes/agregarItemPedidoCotizacion.php',
                        type: type,
                        dataType: 'text',
                        async: true,
                        timeout: duracion,
                        data: { 'idSolicitud': idSolicitud, 'idItem': idItem, 'idItemCotizacion': idItemCotizacion, 'accion': accion, 'cantAprobada': cantAprobada },
                        success: function (datos) {
...
function verificacantAprob(valor) {

    if (valor == "" || valor <= 0) $('#msjeerror').html('<small style="color:red;">La cantidad ingresada no puede ser vacia o menor a 1.</small>');
    else $('#msjeerror').html('');
}


function sincronizarANMAT(evt) {



    bootbox.confirm({
        message: '<div class="alert alert-danger" role="alert"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <b>ATENCIÓN:</b> Realice esta acción <b><u>SOLAMENTE</u></b> si al momento de seleccionar e informar los productos se produce un error y este indica que los productos ya se encuentran informados como RECEPCIONADO/DISPENSADO en ANMAT. </div> <br/> <h4>¿Esta seguro de realizar la acción?</h4>',
        buttons: {
            confirm: {
                label: 'Sí',
        ...
        callback: function (result) {
            if (result) {
                ...
                $("#modalVerSolicitud").loading({
                    stoppable: false,
                    message: "Aguarde un instante, sincronización en curso...",
                });
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/sincronizarANMAT.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
                    data: { 'idSolicitud': idSolicitud, 'evt': evt },
                    success: function (data) {
                        $("#modalVerSolicitud").loading('stop');
                        if (data.trim() == "") {
                            bootbox.alert('Sincronización Finalizada. Verifique resultados y registros de errores en la lista de abajo.');
                            if (evt == 'R')
                                buscarInformados();
                            else {
                                if (evt == 'D') {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/quitarItemPedido.php',
                    type: type,
                    dataType: 'text',
                    async: true,
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/finalizarCotizacion.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function rechazarCotizaciones(idSolicitud) {
    ...
    mensaje2 = '<label>Motivo (Obligatorio):</label> <br/> <textarea id="obsFin" name="obsFin" class="form-control" required></textarea> ' +
        ' <div id="msjeerror"></div><br/> <br/>' +
        '<div class="alert btn-dark" role="alert">  ' +
        '¿Confirma Rechazar la cotización?' +
        '</div> ';
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/rechazarCotizacion.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
...
function enviarCorreoProv(tipo) {

    bootbox.confirm({
        message: "¿Confirma el envío de los correos a los proveedores?",
        buttons: {
            confirm: {
                label: 'Sí',
                className: 'btn-primary'
        ...
        callback: function (result) {
            if (result) {
                ...
                type = 'POST'
                $.ajax({
                    url: 'controller/solicitudes/reenviarEmailsProveedores.php',
                    type: type,
                    dataType: 'text',
                    async: true,
                    timeout: duracion,
                    data: { 'idSolicitud': idSolicitud, 'tipo': tipo },
                    success: function (data) {


                        if (data === "1")
                            notificar('Correos enviados correctamente!');
                        else
                            notificar('Se ha producido un error, por favor intentente nuevamente!');
...
function verHistorialEmails() {
    var idSolicitud = $("#idSolicitud").val();

    var duracion = 180000;
    type = 'POST'
    $.ajax({
        url: 'controller/solicitudes/cargarEmailsEnviados.php',
        type: type,
        dataType: 'text',
        async: true,
        timeout: duracion,
