...
class materiales_solicitudes
{
	...
	public function SelectAllFiltros($fd, $fh, $pr, $es, $idS, $be, $b24, $pd, $nr, $sur, $ur, $cat, $ts,$ex, $prov, $cf)
	{
		global $dblink;
		...
		$query="SELECT DISTINCT s.id,s.fecha, concat(ifnull(p.apellido,''),' ', ifnull(p.nombre,'')) as nombre, p.apellido, p.nombre nombres, p.dni, pd.nombre as puntoDispensa,
								case when est.id <> 34 then est.nombre else
								concat(est.nombre,
								...
								CONVERT((SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id and id_estados<>42), CHAR),
								' de ',
								CONVERT((SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id), CHAR),
								' Cotizado(s) </small>'
								) end
								as estado,

								p.esB24, p.codigoB24, s.esSur, s.idCategoria, s.urgente, s.id_tipo_solicitud, e.id_estados ,
								(select nombre from solicitudes_categorias sc where sc.id=s.idCategoria ) categoria,
								case when est.id=34 and s.fecha_vigencia_cotiz is not null then (case when s.fecha_vigencia_cotiz > now() then '' else ' Vencido! ' end) else '' end as vigente,
								CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id) else 0 end, CHAR) as provAsignados,
								CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id and id_estados<>42) else 0 end, CHAR) as provCotizados
								FROM `materiales_solicitudes` s
					inner join personas p on s.id_personas=p.id
					inner join puntos_dispensa pd on pd.id=s.id_puntos_dispensa
					inner join (
		...
		if ($idS == '') {
			...
			if ($fd != "" && $fh != "") {
				$query .= " and s.fecha between '" . fecha2($fd) . "' and '" . fecha2($fh) . "' ";
			} 
			...
			if ($es != '-1' and $es != 'null'  and $es!='undefined') {
				$query .= "and e.id_estados in (" . $es . ')';
			}
			...
			if($prov!='-1'){
				$query.="AND s.id IN (
					SELECT mcsp.id_solicitudes
					FROM materiales_cotizacion_solic_prov mcsp
					WHERE mcsp.id_solicitudes=s.id AND mcsp.id_proveedores='".$prov."'
			...
			if($cf!='-1'){
				...
				$query.=" AND ( ( CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id) else 0 end, CHAR)=CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id and id_estados<>42) else 0 end, CHAR) 
				AND CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id and id_estados<>42) else 0 end, CHAR)>0 ) OR ((case when est.id=34 and s.fecha_vigencia_cotiz is not null then (case when s.fecha_vigencia_cotiz > now() then '1' ELSE '0' end) ELSE '1' END)=0))";
			}
		} else {
			$query .= ' and s.id=' . $idS . ' ';
		}
	...
	public function SelectAllFiltrosFact($fd, $fh, $pr, $es, $idS, $be, $b24, $pd, $nr, $sur, $ur, $cat, $ts,$ex, $prov, $cf)
	{
		global $dblink;
		...
		$query="SELECT DISTINCT s.id,s.fecha, concat(ifnull(p.apellido,''),' ', ifnull(p.nombre,'')) as nombre, p.apellido, p.nombre nombres, p.dni, pd.nombre as puntoDispensa,
					...
					CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id) else 0 end, CHAR) as provAsignados,
					CONVERT(case when est.id=34 then (SELECT count(1) FROM materiales_cotizacion_solic_prov where id_solicitudes=s.id and id_estados<>42) else 0 end, CHAR) as provCotizados,
					prov.nombre nomProv,  (SELECT ifnull(SUM(IFNULL(mci.cantidadAprob,0)*IFNULL(mci.importe_unitario,0)),0) subtotal
													from materiales_solicitudes_items msi
														 LEFT JOIN materiales_cotizacion_items mci ON mci.id_item=msi.id
													WHERE msi.id_solicitudes=s.id AND mci.id_proveedores=mcspv.id_proveedores) totalSol
					FROM `materiales_solicitudes` s
			inner join personas p on s.id_personas=p.id
			inner join puntos_dispensa pd on pd.id=s.id_puntos_dispensa
			inner join (
			...
			inner join estados est on est.id=e.id_estados 
			INNER JOIN materiales_cotizacion_solic_prov mcspv ON mcspv.id_solicitudes=s.id
			INNER JOIN proveedores prov ON prov.id=mcspv.id_proveedores
		...
		if ($idS == '') {
			...
			if($prov!='-1'){
			   ...
			   $query.=" and mcspv.id_proveedores=".$prov;
			}
	...
	public function SelectExcel($fd, $fh, $pr, $es, $idS, $be, $pd, $sur)
	{
		...
		$query = "SELECT DISTINCT ms.id idSolicitud, ms.id_personas, p.apellido, p.nombre, p.dni, DATE_FORMAT(ms.fecha,'%d/%m/%Y') fechaS, ms.esSur, ms.observaciones,  ms.id_puntos_dispensa, pd.nombre ptoDispensa,
							...
							concat('<table>',
													GROUP_CONCAT(
													DISTINCT IFNULL(
												CONCAT('<tr><td> <b></b>',msi.cantidad,'</td><td> <b></b> ',mp.nombre,'</td><td>', pv.nombre ,'</td><td> <b></b>',mci.cantidadAprob,'</td><td> <b></b>',est2.nombre,'</td><td> <b></b>',mci.importe_unitario,'</td></tr>')
														, '<tr><td></td><td>Sin Productos Solicitados</td></tr>')      
												ORDER BY mp.nombre  SEPARATOR' '),'</table>') AS Productos,
												SUM(mci.cantidadAprob*mci.importe_unitario)	cantAprob
					FROM materiales_solicitudes ms
						...
						INNER JOIN ( 
							...
							LEFT JOIN materiales_cotizacion_solic_prov mcspv ON mcspv.id_solicitudes=ms.id
							LEFT JOIN materiales_cotizacion_items mci ON (mci.id_item=msi.id AND mcspv.id_proveedores=mci.id_proveedores)
							LEFT JOIN proveedores pv ON pv.id=mci.id_proveedores
							INNER JOIN estados est2 ON est2.id=mci.id_estados
