...
/*		$query = "SELECT DISTINCT sit.*, p.nombre nombreProd, p.presentacion, p.gtin gtinProd, 
 ...
 WHERE (1=1) AND site.fhasta IS NULL
	   ...
	   $query="SELECT DISTINCT sit.*, e.nombre estado, site.id idTrazaEstado, site.id_estado, date_format(sit.fechaVenc,'%d/%m/%Y') fechaV, date_format(sit.fechaRemito,'%d/%m/%Y') fechaR,
	                           (SELECT se.id_estados from solicitudes_estados se WHERE se.id_solicitudes=s.id ORDER BY se.fechaAlta DESC LIMIT 1) idEstadoS, date_format(site.fDesde,'%d/%m/%Y %H:%i:%s') fechaD, ifnull(pr.nombre, sit.nombre) nombrePr
				FROM solicitudes_items_traza sit 
	...
	public function Save()
	{
		...
		$fechaRemito=mysqli_real_escape_string($dblink,$this->getfechaRemito());
		if($fechaRemito!=null &&  $fechaRemito!=""){
			$set.=" fechaRemito='".$fechaRemito."',";
		}
	...
	public function Create()
	{
		...
		$query ="INSERT INTO solicitudes_items_traza (	`id_solicitud` ,
                            ...
                            `fechaAlta`) VALUES (" . mysqli_real_escape_string($dblink,$this->getid_solicitud()) .",".
												...
												mysqli_real_escape_string($dblink,$this->getfechaRemito())."',".
                                                $idRecepcion.",".
                                                $idDispensa.",".
	...
	public function setfechaRemito($fechaRemito='')
	{
		$this->fechaRemito = $fechaRemito;
		return true;
	}

	public function getfechaRemito()
	{
