...
class solicitudes
{
	...
	public function SelectExcel($fd, $fh, $pr, $es, $idS, $be,$b24, $pd){
		global $dblink;
		...
		$query ="SELECT  s.id idSolicitud, s.id_personas, date_format(s.fecha, '%d/%m/%Y') fechaS, s.observaciones, s.esB24, s.id_puntos_dispensa, pd.nombre ptoDispensa,
					p.apellido, p.nombre, p.dni, se.id_estados, e.nombre estadoSol,  
					concat('<table>',
						GROUP_CONCAT(
						DISTINCT IFNULL(
					...
					GROUP_CONCAT(
					DISTINCT IFNULL(
					CONCAT('<tr><td> <b>Nombre</b>: ',traza.nombre,'</td><td> <b>GTIN</b>: ', traza.gtin,'</td><td> <b># Serie</b>: ', traza.nroSerie,'</td><td> <b>Lote:</b>', traza.lote,'</td><td> <b>F. Venc.:</b>', DATE_FORMAT(traza.fechaVenc,'%d/%m/%Y'),'</td><td> <b>Remito:</b> ', traza.nroRemito,'</td><td> <b>F. Remito:</b> ', DATE_FORMAT(traza.fechaRemito,'%d/%m/%Y'),'</td><td> <b>Estado:</b> ', estados.nombre,'</td></tr>')
				...
				FROM solicitudes s
						...
						LEFT JOIN `solicitudes_items_traza` traza on length(traza.nroRemito)>1 and traza.id_solicitud=s.id
				...
				GROUP BY s.id, s.id_personas, date_format(s.fecha, '%d/%m/%Y'), s.observaciones, s.esB24, s.id_puntos_dispensa, pd.nombre,
