	...
	*/
	...
	public function updateCotizacion()
	{
		...
		$query = "UPDATE materiales_cotizacion_solic_prov SET 
                        ...
                        `userModif` = '" . $_SESSION['user'] . "',
						`fechaModif` = now() 
						WHERE `id_solicitudes`='{$this->getid_solicitudes()}' and id_proveedores='{$this->getid_proveedores()}'";
	...
	{
		Global $dblink;
		$query = " SELECT c.*, p.nombre nombreProv, p.email, e.id idEStado, e.nombre nombreEst, p.enviarEmail
					FROM  materiales_cotizacion_solic_prov c
						INNER JOIN proveedores p ON p.id=c.id_proveedores
						INNER JOIN estados e ON e.id=c.id_estados
	...
	public function getCotizacionesAprobada()
	{
		Global $dblink;
		...
		$query = " SELECT csp.*, pr.*
					FROM materiales_cotizacion_solic_prov csp
					     INNER JOIN proveedores pr on csp.id_proveedores=pr.id
					WHERE	csp.id_solicitudes='{$this->getid_solicitudes()}' AND id_estados IN (10,45)";
