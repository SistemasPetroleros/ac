...
Global $dblink;
	...
	{
        Global $dblink;
		$query = "SELECT * FROM materiales_productos ORDER BY nombre";
	...
	public function SelectAllOrtopedia($idSolicitud)
	{
        Global $dblink;
		$query = "SELECT *
					FROM materiales_productos_max pm
								INNER JOIN materiales_productos p ON pm.idProducto=p.id
					WHERE idProducto NOT IN (
									SELECT id_producto
									FROM materiales_solicitudes_items msi
									WHERE msi.id_solicitudes=".$idSolicitud."
								)	 
					ORDER BY p.nombre";
	...
	public function getProductoMax($id)
	{
		Global $dblink;
		$query = "SELECT *
					FROM materiales_productos_max pm
