...
class solicitudes
{
	...
	public function SelectAllFiltros($fd, $fh, $pr, $es, $idS, $be, $b24, $pd, $nr, $sur)
	{
		global $dblink;


		$query = "SELECT DISTINCT s.id,s.fecha, concat(ifnull(p.apellido,''),' ', ifnull(p.nombre,'')) as nombre, p.apellido, p.nombre nombres,  p.dni, 
		          pd.nombre as puntoDispensa, est.nombre as estado, p.esB24, p.codigoB24, s.esSur
				FROM `solicitudes` s
	...
	public function SelectExcel($fd, $fh, $pr, $es, $idS, $be, $b24, $pd, $nr, $sur)
	{
		...
		SELECT  s.id idSolicitud, s.id_personas, date_format(s.fecha, '%d/%m/%Y') fechaS, s.observaciones, s.esSur, s.esB24, s.id_puntos_dispensa, pd.nombre ptoDispensa,
							p.apellido, p.nombre, p.dni, se.id_estados, e.nombre estadoSol,  traza.laboratorio,
							concat('<table>',
								GROUP_CONCAT(
								DISTINCT IFNULL(
						...
						FROM solicitudes s
								...
								LEFT JOIN solicitudes_remito sr ON (sr.id_solicitud=s.id )
								LEFT JOIN `solicitudes_items_traza` traza ON (length(traza.nroRemito)>1 and traza.id_solicitud=s.id AND sr.nroRemito=traza.nroRemito)
	...
	{
		global $dblink;
		$query = "INSERT INTO solicitudes (`id_personas`,`fecha`,`observaciones`,`id_puntos_dispensa`,`esB24`, `esSur`) VALUES ('" . mysqli_real_escape_string($dblink, $this->getid_personas()) . "',now(),'" . mysqli_real_escape_string($dblink, $this->getobservaciones()) . "','" . mysqli_real_escape_string($dblink, $this->getid_puntos_dispensa()) . "','" . mysqli_real_escape_string($dblink, $this->getesB24()) . "','" . mysqli_real_escape_string($dblink, $this->getesSur()) . "');";
		mysqli_query($dblink, $query);
