...
class solicitudes_estados
{
	...
	*/
	...
	{
		Global $dblink;
		$query = "SELECT se.*, e.nombre as estado FROM solicitudes_estados se inner join estados e on e.id=se.id_estados WHERE se.`id_solicitudes`='{$this->getid_solicitudes()}' order by fechaAlta desc";
	...
	public function SelectStatusRecent($idSolicitud)
	{
		Global $dblink;
		$query = "  SELECT *
                    FROM solicitudes_estados as T1
                    WHERE NOT EXISTS (
                                        SELECT *
                                        FROM solicitudes_estados as T2
