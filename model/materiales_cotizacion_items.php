...
class materiales_cotizacion_item
{
    ...
    private function Load()
    {
        ...
        $query = "SELECT * FROM materiales_cotizacion_items WHERE `id`='{$this->getid()}'";
    ...
    {
        global $dblink;
        $query = "INSERT INTO materiales_cotizacion_items (	`id_item`,
                                                `id_proveedores`,
                                                `importe_unitario`,
                                                `cantidad`,
                                                `marca`,
    ...
    public function save()
    {
        ...
        $query = "UPDATE materiales_cotizacion_items 
                  SET " . $set . "
    ...
    public function devolverEstadoItems($data)
    {
        ...
        $sql = " SELECT ci.id_estados 
                FROM materiales_cotizacion_items ci 
                INNER JOIN materiales_cotizacion_solic_prov csp ON csp.id_proveedores=ci.id_proveedores
                INNER JOIN materiales_solicitudes_items si  ON si.id=ci.id_item
        ...
        $sql = "SELECT  mci.id_estados
                    FROM materiales_solicitudes_items si
                        INNER JOIN materiales_cotizacion_items mci ON si.id=mci.id_item
                    WHERE si.id_solicitudes=" . mysqli_real_escape_string($dblink, $data['idSolicitud']) . " AND mci.id_proveedores=" . mysqli_real_escape_string($dblink, $data['idProveedor']) . "
                    LIMIT 0,1";
    ...
    public function cantidadTotalAprobada($idItem)
    {
        ...
        $sql = " SELECT ifnull(SUM(cantidadAprob),0) suma, ifnull(si.cantidad,0) cantidad
                    FROM materiales_cotizacion_items  ci
                        INNER JOIN materiales_solicitudes_items si ON ci.id_item=si.id
    ...
    { 
        ...
        /*  $sql = " SELECT si.id idItem, si.id_solicitudes, si.id_producto, p.nombre, si.cantidad cantSolicitada, ci.id idItemCot,
                            ...
                            (ci.cantidad*ci.importe_unitario) total, (ci.cantidadAprob*ci.importe_unitario) totalAprob
                    FROM materiales_solicitudes_items si
                        INNER JOIN materiales_productos p ON si.id_producto=p.id
                        INNER JOIN materiales_cotizacion_items ci ON ci.id_item=si.id
        ...
        $sql = " SELECT si.id idItem, si.id_solicitudes, si.id_producto, p.nombre, si.cantidad cantSolicitada, ci.id idItemCot,
                        ci.id_proveedores, ci.importe_unitario, ci.cantidad cantCotizada, ci.cantidadAprob, ci.marca, ci.id_estados, pv.nombre proveedor,
                        (ci.cantidad*ci.importe_unitario) total, (ci.cantidadAprob*ci.importe_unitario) totalAprob, mc.id idCotizacion
                FROM materiales_solicitudes_items si
                            INNER JOIN materiales_productos p ON si.id_producto=p.id
                            INNER JOIN materiales_cotizacion_items ci ON ci.id_item=si.id
                            INNER JOIN proveedores pv ON pv.id=ci.id_proveedores
                            INNER JOIN materiales_cotizacion_solic_prov mc ON (mc.id_solicitudes=si.id_solicitudes AND mc.id_proveedores=pv.id)
                WHERE si.id_solicitudes=" . mysqli_real_escape_string($dblink, $idSolicitud) . " AND ci.cantidadAprob IS NOT NULL ";
    ...
    public function getItemCotizacion($data)
    {
        global $dblink;

        $sql = " SELECT si.id idItem, si.id_solicitudes, si.id_producto, p.nombre, si.cantidad cantSolicitada, ci.id idItemCot,
                        ci.id_proveedores, ci.importe_unitario, ci.cantidad cantCotizada, ci.cantidadAprob, ci.marca, ci.id_estados, pv.nombre proveedor,
                        (ci.cantidad*ci.importe_unitario) total, (ci.cantidadAprob*ci.importe_unitario) totalAprob, mc.id idCotizacion, mpm.montoMax
                FROM materiales_solicitudes_items si
                            INNER JOIN materiales_productos p ON si.id_producto=p.id
                            INNER JOIN materiales_cotizacion_items ci ON ci.id_item=si.id
                            INNER JOIN proveedores pv ON pv.id=ci.id_proveedores
                            INNER JOIN materiales_cotizacion_solic_prov mc ON (mc.id_solicitudes=si.id_solicitudes AND mc.id_proveedores=pv.id)
                            INNER JOIN materiales_productos_max mpm ON (si.id_producto=mpm.idProducto)
                WHERE si.id_solicitudes=" . mysqli_real_escape_string($dblink, $data['idSolicitud']) . "  AND si.id=" . $data['idItem'];
    ...
    public function  getItemsCotizacion($idSolicitud, $idProveedor)
    {
        ...
        /*  $sql = "SELECT si.id idItem, si.id_solicitudes, si.id_producto, p.nombre,  si.cantidad cantSolicitada, ci.id idItemCot,
                        ci.id_proveedores, ci.importe_unitario, ci.cantidad cantCotizada, ci.cantidadAprob, ci.marca, ci.id_estados, pv.nombre proveedor,
                        (ci.cantidadAprob*ci.importe_unitario) total, mc.id idCotizacion
                FROM materiales_solicitudes_items si
                    INNER JOIN materiales_productos p ON si.id_producto=p.id
                    INNER JOIN materiales_cotizacion_items ci ON ci.id_item=si.id
                    INNER JOIN proveedores pv ON pv.id=ci.id_proveedores
    ...
    public function getItemsCotizacionPorEstado($idSolicitud,$idEstado,$idProveedor)
    {
        global $dblink;
        ...
        $sql = "SELECT si.id idItem, si.id_solicitudes, si.id_producto, p.nombre, si.cantidad cantSolicitada, ci.id idItemCot,
                ...
                FROM materiales_solicitudes_items si
                            ...
                            INNER JOIN proveedores pv ON pv.id=ci.id_proveedores
                            INNER JOIN materiales_cotizacion_solic_prov mc ON (mc.id_solicitudes=si.id_solicitudes AND mc.id_proveedores=pv.id)
                WHERE si.id_solicitudes=".mysqli_real_escape_string($dblink, $idSolicitud)." AND ci.id_estados=".mysqli_real_escape_string($dblink, $idEstado) ." AND pv.id=".mysqli_real_escape_string($dblink, $idProveedor);
    ...
    public function eliminarItemCotizacion($id)
    {
        ...
        $sql = " DELETE FROM materiales_cotizacion_items WHERE  id=" . mysqli_real_escape_string($dblink, $id);
    ...
    public function insertarItemsCotizacionesPorQuery($idSolicitud, $idProveedor)
    {
        ...
        $sql = " INSERT INTO materiales_cotizacion_items (  `id_item`,
                                                            `id_proveedores`,
                                                            `importe_unitario`,
                                                            `cantidad` ,
                                                            `cantidadAprob` ,
                    ...
                    FROM  materiales_cotizacion_solic_prov mc
                          INNER JOIN materiales_solicitudes_items mi ON mc.id_solicitudes=mi.id_solicitudes
