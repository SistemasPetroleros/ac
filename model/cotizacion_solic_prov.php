...
class cotizacion_solic_prov
{
	...
	{
		global $dblink;
		$query = "INSERT INTO cotizacion_solic_prov (`id_proveedores`, `id_solicitudes`, `id_estados`,`userAlta`) VALUES ('" . mysqli_real_escape_string($dblink, $this->getid_proveedores()) . "','" . mysqli_real_escape_string($dblink, $this->getid_solicitudes()) . "','" .mysqli_real_escape_string($dblink, $this->getid_estados())."','". mysqli_real_escape_string($dblink, $this->getuserAlta()) . "');";
	...
	public function Save()
	{
		...
		if ($this->getimporte() != "" and  $this->getimporte() != null) {
			$set .= "`importe` = " . mysqli_real_escape_string($dblink, $this->getimporte()) . ", ";
		}
	...
	public function SelectACotizacionesUserProveedor($data)
	{
		...
		if ($data['idEstadoBuscar'] != "-1" and $data['idEstadoBuscar'] != "null") {
			$where .= " and c.id_estados in (" . mysqli_real_escape_string($dblink, $data['idEstadoBuscar']) . ")";
		}
		...
		$query = "SELECT DISTINCT s.id, s.fecha,  CONCAT(p.apellido,' ', p.nombre) nombre , p.dni, pd.nombre puntoDispensa, pd.gln, l.nombre localidad, prov.nombre provincia,
							...
							s.fecha_vigencia_cotiz, IF(s.fecha_vigencia_cotiz<NOW() AND c.id_estados=11,1,0) excluida, ec.nombre estadoGral,  s.id_tipo_solicitud, (SELECT ifnull(SUM(IFNULL(mci.cantidadAprob,0)*IFNULL(mci.importe_unitario,0)),0) subtotal
							from solicitudes_items msi
								 LEFT JOIN cotizacion_items mci ON mci.id_item=msi.id
							WHERE msi.id_solicitudes=s.id and mci.id_proveedores=up.id_proveedor) totalSol, s.esB24, p.codigoB24
					FROM solicitudes s
					...
					WHERE  e.id_estados NOT IN (1,2,9) AND s.fecha>= STR_TO_DATE('2023-05-01','%Y-%m-%d')  AND u.id=" . $_COOKIE['idUsuario'] . $where . " 
					UNION
					SELECT DISTINCT s.id, s.fecha,  CONCAT(p.apellido,' ', p.nombre) nombre , p.dni, pd.nombre puntoDispensa, pd.gln, l.nombre localidad, prov.nombre provincia,
							up.id_proveedor, (SELECT prr.nombre FROM proveedores prr WHERE prr.id=up.id_proveedor) nomProv, c.id idCotizacion, c.id_estados, est.nombre estadoCot,
							s.fecha_vigencia_cotiz,(s.fecha_vigencia_cotiz<NOW() AND c.id_estados=11) excluida, ec.nombre estadoGral,  s.id_tipo_solicitud, 
							(SELECT ifnull(SUM(IFNULL(mci.cantidadAprob,0)*IFNULL(mci.importe_unitario,0)),0) subtotal
							from materiales_solicitudes_items msi
								 LEFT JOIN materiales_cotizacion_items mci ON mci.id_item=msi.id
							WHERE msi.id_solicitudes=s.id and mci.id_proveedores=up.id_proveedor) totalSol, '0' as esB24, p.codigoB24
					FROM materiales_solicitudes s
						...
						inner join estados ec on ec.id=c.id_estados
						INNER JOIN estados est ON est.id=c.id_estados 
						WHERE (1=1) AND s.fecha>= STR_TO_DATE('2023-05-01','%Y-%m-%d')   AND (ur.id_roles =( select id_roles from usuarios_roles
																												where id_usuarios in (
																												select id from usuarios where user in (
																												select userAlta from materiales_solicitudes_estados
																													where id_solicitudes=s.id and fechaAlta in (
																													select min(fechaAlta)
																														from materiales_solicitudes_estados
																														where id_solicitudes=s.id
																													)
																													)
																												)
																													) OR e.id_estados NOT IN(31,32,33,36))
						AND u.id=" . $_COOKIE['idUsuario'] . " " . $where;
	...
	{
		global $dblink;
		$query = " SELECT c.*, p.id idProveedor, p.nombre nombreProv, p.email, e.id idEStado, e.nombre nombreEst, p.enviarEmail
	...
	public function SelectAllCotizacionesFiltros($data)
	{
		...
		$query = " SELECT c.*, p.id idProveedor, p.nombre nombreProv, p.email, e.id idEStado, e.nombre nombreEst, ms.id_tipo_solicitud, ts.nombre tipo
					FROM  cotizacion_solic_prov c
						INNER JOIN proveedores p ON p.id=c.id_proveedores
						INNER JOIN estados e ON e.id=c.id_estados
						INNER JOIN solicitudes ms on ms.id=c.id_solicitudes
	...
	public function SelectCotizacionAprobada($idSolicitud = '')
	{
		...
		$query = "  SELECT s.id idSolicitud, s.esB24, pd.nombre farmacia, pd.GLN, pd.domicilio, pd.telefonos,
							...
							pr.email, p.dni, p.apellido, p.nombre, p.codigoB24, csp.importe, p.email emailAf, p.telefono
					FROM cotizacion_solic_prov csp
