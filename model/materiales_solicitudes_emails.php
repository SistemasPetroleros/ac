...
/******************************************************************************
* Class for ospepri_altocosto.materiales_solicitudes_emails
*******************************************************************************/

class materiales_solicitudes_emails
{
	/**
	* @var 
	* Class Unique ID
	...
	*/
	...
	{
		Global $dblink;
		$query = "SELECT * FROM materiales_solicitudes_emails WHERE `id`='{$this->getid()}'";
	...
	public function SelectAll()
	{
		...
		$query = "SELECT *, DATE_FORMAT(fechaAlta, '%d/%m/%Y %H:%i:%s') fAlta FROM materiales_solicitudes_emails WHERE id_solicitudes='".mysqli_real_escape_string($dblink,$this->getid_solicitudes())."' AND tipo='".mysqli_real_escape_string($dblink,$this->gettipo())."' ORDER BY fechaAlta";
	...
	{
		Global $dblink;
		$query ="INSERT INTO materiales_solicitudes_emails (`id_solicitudes`,`tipo`, `ok`, `descripcion`, `userAlta`) 
	...
	public function ultimoRegistro()
	{
		...
		$query = "  SELECT *
					FROM materiales_solicitudes_emails se
